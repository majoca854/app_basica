...
#realiazar los reemolazos en las columnas DEPARTAMENTOS Y MINICIPIOS
...
#Crear una condicion negativa para filtrar los departamentos no deseados
...
#Atributos del Datase

filas = df.shape[0]
variables = df.shape[1]
num_deptos =df_colombia_continental['DEPARTAMENTO'].nunique()
num_mpios=df_colombia_continental['MUNICIPIO'].nunique()
                                  
#datos para el grafico interactivo
df_depto_anios = df_colombia_continental.groupby(['DEPARTAMENTO','AÑO SERVICIO'])['ENERGÍA ACTIVA'].sum()
departamentos = df_colombia_continental['DEPARTAMENTO'].unique().tolist()



#Calculo de total por año de Energia Activa
...
st.image('Zonas_no_interconectadas.png')
